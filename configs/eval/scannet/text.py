"""WildDet3D + LingBot-Depth on ScanNet - Text mode zero-shot evaluation.

Evaluates WildDet3D on ScanNet val, zero-shot (the model is trained on
Omni3D + ITW; ScanNet is held out). Reports the standard
Detect3DEvaluator metrics on 18 categories with a Base/Novel split
based on 15 common indoor categories.
...
def get_config() -> ExperimentConfig:
    """WildDet3D + LingBot text eval on ScanNet."""
    config = get_default_cfg(
        exp_name="wilddet3d_lingbot_depth_freeze21_scannet"
    )
