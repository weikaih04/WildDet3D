"""WildDet3D + LingBot-Depth on Argoverse2 - Text mode zero-shot evaluation.

Evaluates WildDet3D on Argoverse2 val, zero-shot (the model is trained
on Omni3D + ITW; Argoverse2 is held out). Reports the standard
Detect3DEvaluator metrics with a Base/Novel split based on the 11
common driving categories (regular vehicle, pedestrian, bicyclist,
construction cone/barrel, large vehicle, bus, truck, vehicular trailer,
bicycle, motorcycle).
...
from configs.base.dataset.open import get_av2_data_cfg
...
def get_config() -> ExperimentConfig:
    """WildDet3D + LingBot text eval on Argoverse2."""
    config = get_default_cfg(
        exp_name="wilddet3d_lingbot_depth_freeze21_argoverse"
    )
    ...
    argoverse_test_data_cfg = get_av2_data_cfg(
        data_backend=data_backend,
        shape=sam3_image_shape,
    )
    ...
    config.data = get_wilddet3d_data_cfg(
        ...
        test_datasets=[argoverse_test_data_cfg],
    ...
    evaluators = [get_av2_evaluator_cfg()]
